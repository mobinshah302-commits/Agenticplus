...
def _safe_join(home: str, relpath: str) -> str:
    """جلوگیری از خروج از هوم کاربر با ../ یا مسیر مطلق."""
    relpath = (relpath or "").lstrip("/")
    target = os.path.normpath(os.path.join(home, relpath))
    if not (target == home or target.startswith(home + os.sep)):
        raise ValueError("مسیر غیرمجاز")
    return target


def list_workspace_files(user_id: int, subpath: str = "", max_items: int = 100) -> list[dict]:
    """لیست فایل/پوشه‌های داخل هوم کاربر (یا زیرپوشه‌ی داده‌شده)."""
    home = workspace_path(user_id)
    base = _safe_join(home, subpath)
    if not os.path.isdir(base):
        return []
    items = []
    for entry in sorted(os.listdir(base))[:max_items]:
        ...
        if os.path.isdir(full):
            ...
            try:
                ...
                size = 0
            items.append({"name": rel, "type": "file", "size": size})
    return items


def read_workspace_file(user_id: int, relpath: str, max_bytes: int = 45 * 1024 * 1024) -> bytes:
    """خوندن یک فایل از هوم کاربر برای ارسال به تلگرام. سقف حجم به خاطر محدودیت آپلود بات."""
    home = workspace_path(user_id)
    full = _safe_join(home, relpath)
    if not os.path.isfile(full):
        raise FileNotFoundError(f"فایل پیدا نشد: {relpath}")
    size = os.path.getsize(full)
    if size > max_bytes:
        raise ValueError(f"فایل خیلی بزرگه ({size // (1024*1024)}MB) — سقف ارسال بات حدود {max_bytes // (1024*1024)}MB")
